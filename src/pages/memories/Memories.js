...
const Memories = () => {
    ...
    const memories = useSelector(state =>{
        ...
        Array.from(state.memories.data).forEach((element,idx) => {
            temp.push({'#':idx+1,id:element.id,'Name':element.name,'Brand':element.brand,'Price':element.product.price,'Stock':element.product.stock?element.product.stock:0,deleted_at:element.deleted_at})
        });
        return temp
    })
