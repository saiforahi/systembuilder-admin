...
const Cases = () => {
    ...
    const cases = useSelector(state =>{
        let temp=[]
        Array.from(state.cases.data).forEach((element,idx) => {
    ...
    const _delete=(storage_id)=>{
        ...
        swal({
          ...
          .then((willDelete) => {
            if (willDelete) {
              API.delete('cases/delete/'+storage_id).then(response=>{
            ...
            <CContainer>
                <CRow className="align-items-center">
                    <CCol md="8" className="mb-3 mb-xl-0 text-left offset-md-2">
                        <Link to="/dashboard/cpucases/create"><CButton shape="round" color="primary">Add</CButton></Link>
                    </CCol>
                </CRow>
                <hr></hr>
                <CRow>
                                    ...
                                    scopedSlots={{
                                        'Action':
                                            (item) => (
                                                <td>
                                                    <CBadge>
                                                        <CButton disabled={is_trashed(item.deleted_at)} onClick={() => _delete(item.id)} type="button" size="sm" color="danger">Delete</CButton> <CButton onClick={() => { history.push({ pathname: '/dashboard/cases/details', state: { brand: item } }) }} size="sm" type="button" color="primary">Edit</CButton>
                                                    </CBadge>
                                                </td>
                                            )
                                    }}
